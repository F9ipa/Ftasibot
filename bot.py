...
def get_signals():
    ...
    def scan(sym):
        try:
            df = yf.download(sym, period="3y", interval="1mo", progress=False)
            if df.empty or len(df) < 5: return
            wt1, wt2 = calculate_wavetrend(df)
            if wt1 is None: return
            c1, p1, c2, p2 = wt1.iloc[-1], wt1.iloc[-2], wt2.iloc[-1], wt2.iloc[-2]
            name = sym.replace(".SR", "")
            price = f"{df['Close'].iloc[-1]:.2f}"
            if p1 <= p2 and c1 > c2: pos.append(f"🟢 {name} ({price})")
            elif p1 >= p2 and c1 < c2: neg.append(f"🔴 {name} ({price})")
        except: pass
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = [[InlineKeyboardButton("📈 إيجابي (دخول)", callback_data='pos')],
          [InlineKeyboardButton("📉 سلبي (خروج)", callback_data='neg')]]
    await update.message.reply_text("مرحباً بك في رادار WaveTrend تاسي (شهري):\nاختر الفلتر:", reply_markup=InlineKeyboardMarkup(kb))
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main':
        kb = [[InlineKeyboardButton("📈 إيجابي (دخول)", callback_data='pos')],
              [InlineKeyboardButton("📉 سلبي (خروج)", callback_data='neg')]]
        await query.edit_message_text("اختر الفلتر (فاصل شهري):", reply_markup=InlineKeyboardMarkup(kb))
        return

    await query.edit_message_text("🔍 جاري فحص كامل السوق... فضلاً انتظر ثوانٍ.")
    pos, neg = get_signals()
    
    res = pos if data == 'pos' else neg
    title = "✅ الإيجابية (دخول):" if data == 'pos' else "❌ السلبية (خروج):"
    
    msg = f"**{title}**\n\n" + ("\n".join(res) if res else "لا توجد إشارات حالياً")
